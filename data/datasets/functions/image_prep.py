...
20x20 digits centered on a 28x28 white background.
"""
...
def function_parser():
	...
	functions = os.path.abspath("datasets/functions/function_data")

	symbols = os.path.abspath("datasets/symbols")
	...
	for index, subfolder in enumerate(os.listdir(symbols)):
		subfolder = os.path.normpath(os.path.join(symbols, subfolder))
		if os.path.isdir(subfolder):
			for filename in os.listdir(subfolder):
				filepath = os.path.join(subfolder, filename)
				_resize_and_center(filepath)
...
def _resize_and_center(filepath):
	...
	new_image = image.resize((basewidth, hsize), Image.LANCZOS)
...
def _trim_whitespace(filepath):
	...
	idx = np.where(image_data-65535)[0:2] # uint-8 "white" 
...
def _center_image(filepath):
	...
	new_size = (28, 28)
	new_image = Image.new("I", new_size, 65535) #
	new_image.paste(interim_image, ((new_size[0]-interim_size[0])/2, (new_size[1]-interim_size[1])/2))
