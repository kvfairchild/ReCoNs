...
def add_nodes(recon, node_data):
	recon.add_layers(node_factory(node_data))
...
def link_nodes(recon, link_data):
	...
	recon.links_list = [[(create_link(recon, link)) 
	for sub_layer in layer for link in sub_layer] for layer in link_data]

def create_link(recon, link_data):
...
def has_sublists(object):
	return any(isinstance(item, list) for item in object)
